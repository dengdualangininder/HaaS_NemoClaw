...
class NimNemotronReasoner(BaseReasoner):
    ...
    def info(self) -> ReasonerInfo:
        return ReasonerInfo(backend="nim", model_name=self.model_name)
...
def build_reasoner(backend: str, model_name: str = DEFAULT_NIM_MODEL) -> BaseReasoner:
    if backend == "scripted_nemotron":
        return ScriptedNemotronReasoner(model_name=model_name)
    if backend == "nim":
        return NimNemotronReasoner(
            model_name=model_name,
            base_url=os.getenv("NIM_BASE_URL", DEFAULT_NIM_BASE_URL),
        )
